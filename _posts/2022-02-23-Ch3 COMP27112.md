...
`OpenGL` 使用 **凸多边形** (`Convex Polygon`), 也就是 **所有内角均大于 $180\degree$** 的多边形. 若遇到 **凹多边形** (`Concave Polygon`), 则 `OpenGL` 无法直接正确地渲染它们, 因此需要先将它们转换为凸多边形.
...
3. 计算 $(-E_1, E_2)$ 的叉乘 $N = E_2 \times -E_1$, 结果 $N$ 的 **单位向量** 就是该多边形的曲面法线. 
