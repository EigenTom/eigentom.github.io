...
### Things make estimation hard: (?)

### Things can help you estimate: (?)
...
### Failing test:
   * fail unit test: fucked
   * failed pre-passed acceptance test: fucked
   * failed acceptance test on future features: it's ok
...
## Week 8: Refactoring and Mgration
...
Caution: No overuse, No over-engineering, refactor TOWARD a pattern rather than TO a pattern (把pattern作为参考, 而不是标准)
