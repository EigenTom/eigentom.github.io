...
DELIMITER //
  ...
  CREATE PROCEDURE myproc(IN p varchar OUT q int INOUT r boolean)
    BEGIN
...
触发器的实质是一段预先存储的 `SQL` 程序, 在监听到发生对应的数据库操作 (如插入, 表更新和表删除) 时会被唤起调用, 一般在 **`SQL` 查询执行前或执行后** 运行.
...
4. 我们可以使用 `FOLLOWS`和 `PRECEDES` 实现触发器的 **链式调用**:
...
Staff(StaffID, StaffForename, StaffSurname, CustomerForename, CustomerSurname, CustomerEmail, CustomerAddressLine1, CustomerTown, CustomerPostCode, dateAdvised) <br> PK[StaffID]
