...
($\uparrow$ 回顾我们在介绍 `Rabin-Miller` 素数判定法时引入的结论: 当且仅当 $n$ 为素数时, 对于 $x^2 ~\equiv~1~\text{mod} (n)$ 有 $x ~\equiv~ \plusmn 1 ~\text{mod}~n$. 在本题中对 $x$ 的取值范围限定了 $x$ 绝不可能满足 $x ~\equiv~ \plusmn 1 ~\text{mod}~n$, 因此 $x$ 必不为素数.)
